...
export default function MiniGrid({ name, cells = [], finished }) {
  ...
  return (
    <div className={`mini-grid-wrap${finished ? ' mini-grid-wrap--done' : ''}`}>
            ...
            <div key={boxRow * 3 + boxCol} className="mini-grid__box">
              {Array.from({ length: 3 }, (_, cellRow) =>
                Array.from({ length: 3 }, (_, cellCol) => {
                  ...
                  const cls =
                    v === 0 ? '' :
                    v > 0   ? ' mini-cell--correct' :
                               ' mini-cell--wrong'
                  return (
                    <div key={cellRow * 3 + cellCol} className={`mini-cell${cls}`} />
                  )
                })
              )}
            </div>
